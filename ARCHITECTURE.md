...
### Auth Tables (Lucia-managed)

- **auth_user** - Lucia-managed. Extended with org_id and email. Lucia owns the schema.
- **auth_session** - Lucia-managed. Session lifecycle handled by Lucia.
- **auth_key** - Lucia-managed. Stores password hashes for email/password auth.

> **Auth library verification required in Week 0:** confirm @lucia-auth/adapter-drizzle compatibility with Drizzle version in use. Lucia v3 has breaking changes from v2. If gaps, better-auth is the fallback — Drizzle-native from day one.

### Application Tables

- **organizations** - id, name, slug (top-level tenant)
- **users** - id, org_id, email, display_name, role (admin/member/viewer). Application table linked to auth_user via FK.
...
- Lucia Auth (self-hosted, MIT, Drizzle/Postgres native). OIDC/SSO via Lucia adapter (Azure AD first).
...
| Auth | Lucia Auth (or better-auth) | Self-hosted, MIT, Drizzle-native. Verified in Week 0 |
