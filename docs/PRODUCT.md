...
- Auth library: better-auth (Lucia deprecated March 2025). Installed and verified.
...
- Week 2: Local auth (better-auth, 1 day). Crawl scheduler (graphile-worker). Job discovery with Bottleneck rate limiter. **Designer delivers three core screen flows by end of Week 2** so frontend hire has specs to scaffold against.
