...
export async function organizationRoutes(app: FastifyInstance) {
  ...
  app.post<{
    Body: { orgName: string; email: string; password: string; displayName: string };
    ...
    if (existingOrgs.length > 0) {
      return reply.status(409).send({
        data: null,
        error: 'Setup already completed. Use /api/auth/sign-in/email to log in.',
      });
    }
    ...
    return reply.status(201).send({
      data: {
        organization: org,
        user,
        nextStep: 'Sign up at /api/auth/sign-up/email with the same email, then add a Jenkins instance at POST /api/v1/instances',
      },
      error: null,
    });
  });
  ...
  app.get('/api/v1/organization', { preHandler: requireAuth }, async (request, reply) => {
    // Get the org for the current user
    const session = request.tigSession;
    if (!session) {
      return reply.status(401).send({ data: null, error: 'Authentication required' });
    }
    ...
    if (!user) {
      return reply.status(404).send({ data: null, error: 'User not found in organization' });
    }
