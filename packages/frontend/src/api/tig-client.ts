...
export const tigSetup = {
  getStatus: () => request<{ isSetUp: boolean; organization: { id: string; name: string } | null }>('/v1/setup/status'),
  create: (orgName: string, email: string, password: string, displayName: string) =>
    request('/v1/setup', {
      method: 'POST',
...
export const tigInstances = {
  list: () => request<{ id: string; name: string; baseUrl: string; isActive: boolean; lastCrawlAt: string | null }[]>('/v1/instances'),
  create: (data: { name: string; baseUrl: string; username: string; token: string; organizationId: string }) =>
    request('/v1/instances', { method: 'POST', body: JSON.stringify(data) }),
  test: (id: string) => request<{ success: boolean; message: string }>(`/v1/instances/${id}/test`, { method: 'POST' }),
};
...
export const tigDashboard = {
  summary: (instanceId?: string) => {
    ...
    return request<{ total: number; passing: number; failing: number; building: number }>(`/v1/dashboard/summary${qs}`);
  },
...
export const tigJobs = {
  list: (instanceId: string, options?: { status?: string; limit?: number; offset?: number }) => {
    const params = new URLSearchParams();
...
export const tigBuilds = {
  ...
  log: (buildId: string) =>
    request<string>(`/v1/builds/${buildId}/log`),
};
