...
import { colors, cardStyle, HEALTH_COLORS, statusGradient } from '../../theme/mantine-theme';
...
function StatCard({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    <Card radius="md" style={cardStyle} p="md">
      <Text size="xs" c={colors.textTertiary} fw={500}>{label}</Text>
      <Text size="xl" fw={700} c={color ?? colors.text} mt={4}>{value}</Text>
    </Card>
  );
}
...
export function DashboardPage() {
  ...
  const h = health.data;
  const healthColor = h ? (HEALTH_COLORS[h.level] ?? colors.textMuted) : colors.textMuted;
    ...
    <Stack gap="lg">
        ...
        <Card radius="md" style={cardStyle} p="sm">
          <Group gap="sm">
            ...
            <Text size="sm" c={colors.textTertiary}>
              {h.score}/100 · {h.agentsOnline}/{h.agentsTotal} agents · {h.queueDepth} queued
            </Text>
          </Group>
        </Card>
      )}
      ...
      {stats && (
        <SimpleGrid cols={{ base: 2, md: 4 }}>
          ...
          <StatCard label="Passing" value={stats.passing} color={colors.success} />
          <StatCard label="Failing" value={stats.failing} color={colors.failure} />
          <StatCard label="Building" value={stats.building} color={colors.info} />
        </SimpleGrid>
      )}
                  ...
                  <Group justify="space-between" wrap="nowrap">
                    <Stack gap={2} style={{ flex: 1, minWidth: 0 }}>
                      <Group gap="xs">
                        ...
                        {g.streak > 1 && (
                          <Badge size="xs" variant="filled" color="red">{g.streak}×</Badge>
                        )}
                      </Group>
                      {aiSummary && (
