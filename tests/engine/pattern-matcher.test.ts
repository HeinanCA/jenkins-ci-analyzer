...
import { analyzeLog } from '../../src/features/build-analysis/engine/pattern-matcher';
import { FAILURE_PATTERNS } from '../../src/features/build-analysis/engine/failure-patterns';
...
describe('analyzeLog', () => {
  ...
  it('detects dependency resolution failures', () => {
    const log = 'npm ERR! 404 Not Found - GET https://registry.npmjs.org/nonexistent-pkg';
    const results = analyzeLog(log, FAILURE_PATTERNS);
    expect(results.some((r) => r.pattern.id === 'dependency')).toBe(true);
  });

  it('detects Docker daemon issues', () => {
    const log = 'Cannot connect to the Docker daemon at unix:///var/run/docker.sock';
    const results = analyzeLog(log, FAILURE_PATTERNS);
    expect(results.some((r) => r.pattern.id === 'docker')).toBe(true);
  });

  it('detects SCM checkout failures', () => {
    const log = "fatal: repository 'https://github.com/org/repo.git/' not found";
    const results = analyzeLog(log, FAILURE_PATTERNS);
  ...
  it('handles very long logs by scanning last 2000 lines', () => {
    const lines = Array.from({ length: 5000 }, (_, i) => `Line ${i + 1}: normal output`);
